...
class ikfk_arm(Ui_MainWindow):
    ...
    def disp_gpos(self):
        desc="Detected.. "
        for i in range(len(self.objek)):
            desc = desc+"\n"+str(i)+": objek: "+str(self.objek[i]['nama'])+" Pos: "+str(self.objek[i]['centerbbox'])
            xpos=self.objek[i]['centerbbox'][0]
            ypos=self.objek[i]['centerbbox'][1]
            x_cal,y_cal=kpos.mapping_cam2real(xpos,ypos)
            self.x_pos.append(float(x_cal/10)) #convert to cm
            self.y_pos.append(float(y_cal/10)) #convert to cm
        print(self.x_pos)
        print(self.y_pos)
    ...
    def konek2robot(self):
        self.ik_state=1
        self.doubleSpinBox_ik1.setProperty("value",self.x_pos[self.ik_state-1])
        self.doubleSpinBox_ik2.setProperty("value",self.y_pos[self.ik_state-1])
        self.doubleSpinBox_ik3.setProperty("value",10)
        #print(self.objek)
    ...
    def ik_next(self):
        ...
        if self.ik_state>0 and self.ik_state<len(self.x_pos):
            self.doubleSpinBox_ik1.setProperty("value",self.x_pos[self.ik_state])
            self.doubleSpinBox_ik2.setProperty("value",self.y_pos[self.ik_state])
            self.doubleSpinBox_ik3.setProperty("value",10)
            self.ik_state+=1
            time.sleep(1)
            self.start_ik()
        if self.ik_state>=len(self.x_pos):
            self.ik_state=0
            self.MplWidget.msgwarning()          
